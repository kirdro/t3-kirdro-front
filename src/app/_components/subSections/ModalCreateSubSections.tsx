...
export const ModalCreateSubSections: FC<IProps> = (props) => {
	const { onClose, idSection, nameSection, sectionPath } = props;
	...
	const { mutate, isPending } = api.subSection.create.useMutation({
		onSuccess: async () => {
			onClose();
	...
	useEffect(() => {
		setPath(`${sectionPath === '/' ? '' : sectionPath}/`);
	}, [sectionPath]);
	...
	return (
		<>
			<ModalHeader className='flex flex-col gap-1'>
				Создание подраздела {nameSection} ({idSection})
			</ModalHeader>
			<ModalBody>
